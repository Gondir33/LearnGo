...
func (g *GeoService) SearchApi(SearchR *SearchRequest) (*SearchResponse, error) {
	...
	if resp, ok := checkCache(SearchR); ok {
		return resp, nil
	}
	...
	for _, address := range addresses {
		respBody.Addresses = append(respBody.Addresses, models.Address{Lat: address.Data.GeoLat, Lng: address.Data.GeoLon})
	}

	err = storage.AddCache(context.TODO(), respBody.Addresses, SearchR.Query)
	if err != nil {
		log.Print(err)
	}
...
func checkCache(SearchR *SearchRequest) (*SearchResponse, bool) {
	resp, err := storage.CheckLevenshtein(context.TODO(), SearchR.Query)
	if err != nil {
		return &SearchResponse{}, false
	}
	return &SearchResponse{Addresses: resp}, true
}
...
func (g *GeoService) GeocodeApi(GeocodeR *GeocodeRequest) (*GeocodeResponse, error) {
	...
	for _, data := range dataSuggestions.Suggestions {
		geocodeResp.Addresses = append(geocodeResp.Addresses,
			models.Address{Lat: fmt.Sprintf("%v", data.Data["geo_lat"]), Lng: fmt.Sprintf("%v", data.Data["geo_lon"])})
	}
