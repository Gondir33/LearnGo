...
type GithubAdapter struct {
	...
	gistsCache map[string][]Item
	reposCache map[string][]Item
	Reposer
	Gister
}
...
func NewGithub(client *github.Client) *GithubAdapter {
	g := &GithubAdapter{
		client:     client,
		gistsCache: make(map[string][]Item),
		reposCache: make(map[string][]Item),
	}
	g.Gister = &Gist{g}
	g.Reposer = &Repos{g}
	return g
...
func (g *GithubAdapter) GetGists(ctx context.Context, username string) ([]Item, error) {
	if got, ok := g.gistsCache[username]; ok {
		return got, nil
	}
	items := make([]Item, 0)
...
func (g *GithubAdapter) GetRepos(ctx context.Context, username string) ([]Item, error) {
	if got, ok := g.reposCache[username]; ok {
		return got, nil
	}
	items := make([]Item, 0)
